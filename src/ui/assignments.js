                ...
                <div class="relative group">
                    ...
                    <div class="relative flex items-center">
                        ...
                        <select id="assignment-subject" onchange="window.autoSuggestAssignmentName()" class="w-full ${inputPadding} py-3 bg-gray-50/50 border-2 border-gray-100 rounded-xl focus:bg-white focus:border-purple-500 focus:ring-4 focus:ring-purple-500/10 outline-none transition-all font-medium text-gray-700 appearance-none cursor-pointer">
                            <option value="">${t.subjectPlaceholder || 'Choisir matière...'}</option>
                    ...
                    <div class="p-4 sm:p-6 flex-1 cursor-pointer select-none bg-gradient-to-br from-white via-white to-gray-50/50" onclick="toggleAccordion('${a.id}')">
                        <div class="flex items-start justify-between gap-3 mb-3 sm:mb-6">
                            <div class="min-w-0 flex-1">
                                ...
                                <div class="flex items-center flex-nowrap gap-2 mt-2 overflow-x-auto no-scrollbar">
                                    <!-- Points -->
                                    <span class="text-[10px] sm:text-sm ${fontBlack} text-blue-600 bg-blue-50 px-1.5 py-0.5 sm:px-2.5 sm:py-1 rounded-lg border border-blue-100 shadow-sm whitespace-nowrap shrink-0">
                                        ${totalPoints} ${t.points}
                                    </span>
                                    
                                    <!-- Progression (Compact on mobile) -->
                                    <span class="flex sm:hidden items-center gap-1 px-1.5 py-0.5 rounded-lg bg-gray-50 text-[10px] font-bold text-gray-500 border border-gray-100 whitespace-nowrap shrink-0">
                                        <div class="w-1 h-1 rounded-full ${completionRate === 100 ? 'bg-green-500' : 'bg-blue-400 animate-pulse'}"></div>
                                        ${nbGrades}/${nbStudents} (${completionRate}%)
                                    </span>

                                    <!-- Visibility (Compact on mobile) -->
                                    <span class="inline-flex sm:hidden items-center shrink-0">
                                        ${a.isVisible ? `<span class="px-1.5 py-0.5 rounded-lg bg-green-50 text-green-700 border border-green-100 flex items-center justify-center shadow-sm"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg></span>` : `<span class="px-1.5 py-0.5 rounded-lg bg-gray-50 text-gray-400 border border-gray-100 flex items-center justify-center shadow-sm"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21"></path></svg></span>`}
                                    </span>
                                </div>
                            </div>
                            <div id="icon-${a.id}" class="w-8 h-8 sm:w-11 sm:h-11 rounded-lg sm:rounded-2xl bg-white shadow-md border border-gray-100 flex items-center justify-center text-gray-400 group-hover:bg-blue-600 group-hover:text-white group-hover:border-blue-600 group-hover:rotate-180 transition-all duration-500 shrink-0">
                                <svg class="w-4 h-4 sm:w-5 sm:h-5 transform transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                            </div>
                        </div>
